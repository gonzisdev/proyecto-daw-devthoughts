...
export const SearchBar = ({ className }) => {
	...
	const handleSearch = async () => {
		// Impedimos la búsqueda si no el campo esta vacio
		if (searchTerm !== "") {
			try {
				// Necesitamos el token para hacer la petición a nuestro backend y pasar el middleware de JWT
				...
				const config = {
					...
					body: JSON.stringify({ term: searchTerm }), // Convertimos a JSON
				};
	...
	const handleChange = (e) => {
		// Colocamos en el state el termino de busqueda
		setSearchTerm(e.target.value);
	};

	const handleSubmit = (e) => {
		// En el submit prevenimos la accion por defecto y llamamos a la funcion de busqueda
		e.preventDefault();
		handleSearch();
	};
