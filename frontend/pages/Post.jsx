...
export const Post = () => {
	...
	useEffect(() => {
		const loadPost = async () => {
			await getPost(params.id); // Obtenemos el post con los params de la URLs
	...
	const validateComment = comment => {
		...
		if (comment.length > 255) return "Un comentario permite como máximo 255 caracteres";
		return "";
	};
	...
	return (
		<>
			...
			{post.id ? (
				...
				// Si no existe post.id (es que hemos borrado el post, o intentamos entrar en un post inexistente) redireccionamos
				<Navigate to="/home" />
			)}
		</>
	);
